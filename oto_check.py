...
POS_3_CLICK            = (1580, 400)    # 6: sol tık
...
POS_SINGLE_AFTER_101   = (1492, 262)   # 14: tek tık (ekran dışı olabilir!)

# >>> Yeni: Çift tık yerine iki farklı konum
POS_CLICK_1            = (826,  750)   # ilk tıklanacak koordinat
POS_CLICK_2            = (1047, 755)   # ikinci tıklanacak koordinat

# Hız / bekleme
"""
MOVE_DURATION = 0.2
DOUBLECLICK_INTERVAL = 0.05
READ_WAIT_AFTER_CLICK = 0.2
MAX_STAGNANT_READS = 20   # sayı artmıyorsa en fazla kaç deneme
"""

# Hız / bekleme (güncellendi)
...
pyautogui.PAUSE = 0.10          # (eski: 0.05)

# Chrome debug port
DEBUGGER_ADDR = "127.0.0.1:9222"

# PyAutoGUI güvenlik
pyautogui.FAILSAFE = True      # imleci sol-üst köşeye taşırsan PyAutoGUI acil durdurur
...
def increment_until_v0(drv, target_first):
    ...
    while True:
        ...
        # >>> Değiştirildi: çift tık yerine iki farklı konuma tek tık
        click_at(*POS_CLICK_1, clicks=1)
        time.sleep(DOUBLECLICK_INTERVAL)
        click_at(*POS_CLICK_2, clicks=1)
    ...
    while True:
        ...
        if (tot is not None) and (tot > 101) and (target_first == 101):
            ...
            if cur == 100:
                ...
                time.sleep(READ_WAIT_AFTER_CLICK)
                print("\n[Bilgi] 100'de son çift tık (iki noktaya) atıldı; sonraki aşamaya geçiliyor.")
                return
            # cur < 100 → çift tık
            # >>> Değiştirildi: çift tık yerine iki farklı konuma tek tık
            click_at(*POS_CLICK_1, clicks=1)
    ...
    try:
        ...
        while True:
            ...
            if cur is None or tot is None:
                print("[Hata] Sayılar okunamadı (CSS/iframe/shadow DOM?). Çalışma durduruldu.")
                break
